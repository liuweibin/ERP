...
public class JsGridReportBase {
	...
	private String modelPath = "../../config/model.xls";//样式模板文件路径
	...
	public HSSFWorkbook writeSheet(HSSFWorkbook wb, String title, HashMap<String, HSSFCellStyle> styles, String creator, TableData tableData) throws Exception {
		...
		if(this.getRemark()!=null){
			...
			cell.setCellValue(new HSSFRichTextString(this.getRemark()));
			
			headerMetaData = tableData.getTableHeader();// 获得HTML的表头元素
			sheet.addMergedRegion(new CellRangeAddress(rownum, rownum, 0, headerMetaData.getColumnCount() - 1));
			rownum++;
		}
	...
	private HashMap<String, HSSFCellStyle> initStyles(HSSFWorkbook wb) {
		...
		try {
			POIFSFileSystem fs = new POIFSFileSystem(getClass()
					.getClassLoader().getResourceAsStream(modelPath));
