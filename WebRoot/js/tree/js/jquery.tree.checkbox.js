...
(function($) {
	...
	$.fn.treeview = function(settings) {
		var dfop = {
			...
			cascadecheck : false, //是否启用级联，默认启用==============之前是true==============
			data : null, //初始化数据
			clicktoggle : false, //点击节点展开和收缩子节点
		...
		function buildnode(nd, ht, deep, path, isend) {
			...
			ht.push("<span unselectable='on' id=" + nid
					+ " class='bbit_tree_node_cb_span'>", nd.text, "</span>");
			ht.push("</a>");
			ht.push("</div>");
			//Child
			if (nd.hasChildren) {
		...
		if (s != null){
			s.join(",");
		document.getElementById("agentsCode").value=s;
		
		}
		else{
			document.getElementById("agentsCode").value=""
				
			}
				//=======================
				//bbit_tree_node_cb_span_o
				//=======================
			}
			//====================================================================

			else if ($(et).hasClass("bbit_tree_node_cb_span")) {
				var s = item.checkstate != 1 ? 1 : 0;//======================================
				var r = true;
				if (dfop.oncheckboxclick) {
					r = dfop.oncheckboxclick.call(et, item, s);
	...
	$.fn.clear = function() {
		if (this[0].t) {
			return this[0].t.clear();
		}
		return null;
	};
	//节点点击
	$.fn.nodeclick = function(item, s) {
		if (this[0].t) {
			this[0].t.check(item, s);
		}
	};
})(jQuery);