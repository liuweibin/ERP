...
$().ready(function() {
 ...
 $(document).ready(function(){ 
   ...
   $.ajax({ 
                ...
                type: "POST",
                async: false,
                url:"<%=basePath%>unionpay/saveOrUpdate.do", 
		...
		<form id="signupForm" method="post" action="" style="text-align: center;" >
			...
			<table width="50%"  class="table">
				<tr style="margin:10px; ">
					...
					<td style="float: left;">
						<input id="acct" name="acct"  class="input6" value="${tlAgentsBankBind.acct}"    /> 
					</td>
					<td align="right">
						<label for="bank_code">
							银行编码 :
					...
					<td>
						...
						--%>
						<select id="bank_code" name="bank_code" class="input6" >
						<option value="" >未知</option> 
						<c:forEach var="data" items="${banksList}">
						<c:if test="${data.bankCode == tlAgentsBankBind.bankCode}">
								<option value="${data.bankCode}" selected>${data.name}</option> 
						</c:if>
						<c:if test="${data.bankCode != tlAgentsBankBind.bankCode}">
								<option value="${data.bankCode}" >${data.name}</option> 
						</c:if>
						</c:forEach>
						</select>
					</td>
				...
				<tr>
					...
					<td>
						<input id="bank_name" name="bank_name" class="input6" value="${tlAgentsBankBind.bankName}" />
					</td>
					<td align="right">
						<label for="ac_name">
					...
					<td style="float: left;" >
						<input type="text" id="ac_name" class="input6" name="ac_name" value="${tlAgentsBankBind.acName}" />
					</td>
				</tr>	 
				<tr>
					<td align="right">
					...
					<td style="float: left;" >
						<input type="province" class="input6" id="province" name="province" value="${tlAgentsBankBind.province}"/>
					</td>
					<td align="right">
						<label for="rightname">
							开户市:
					...
					<td style="float: left;">
						<input id="city" name="city" class="input6" value="${tlAgentsBankBind.city}"/>
					</td>
				</tr>	 
				<tr>
					<td align="right">
					...
					<td style="float: left;">
					<select id="account_type" name="account_type"  class="input6">
					...
					<td style="float: left;">
						<select id="account_prop" name="account_prop"  class="input6">
				...
				<tr>
					...
					<td style="float: left;">
					<select id="id_type" name="id_type" onchange="change()" class="input6" >
					...
					<td style="float: left;">
					<input id="idno" name="idno" class="input6" value="${tlAgentsBankBind.idno}" />
					</td>
				</tr>	<!--  
				<tr>
					<td align="right">
				...
				<tr>
					...
					<td style="float: left;">
					<input id="mobile" name="mobile" class="input6" value="${tlAgentsBankBind.mobile}"  /> 
					</td>
					<c:if test="${type=='edit'}">
					<td align="right">
						<label for="agrdeadline">
