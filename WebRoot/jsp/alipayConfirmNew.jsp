...
function init(){
	$.ajax({
		...
		async : false,
		dataType:'json',
		url:'<%=basePath%>alipay/prepayInit.do',
		data:{},
		...
		if(data.retcode==0){
			...
			$("#WIDdefaultbank").val(bank.bankBriefCode);
			$("#rechargeCode").val(recharge.rechargeCode);
			$(".rechargeCode").html(recharge.rechargeCode);
			$(".paymoney").html(paymoney);
			$(".rechargeMoney").html(recharge.rechargeMoney);
					...
					<table width="100%" border="0" cellpadding="0" cellspacing="0" style="border: 1px solid #efefef">
						<tr>
							<td style="line-height: 28px;">
								<form name="subxyd" method="post" onsubmit="return waitRechargePage();" action="<%=basePath%>alipay/pay.do" target="_bank">
									<input name="WIDsubject" type="hidden" value="购买信用点">
									<input name="WIDbody" type="hidden" value="购买信用点">
									<input name="WIDdefaultbank" type="hidden" id="WIDdefaultbank" value="${bank.bankBriefCode}">

									<br>
									<input name="rechargeCode" type="hidden" id="rechargeCode" value="${recharge.rechargeCode}" />
										...
										<tr>
											<td height="30" align="center">
												<div align="center" style="padding: 20px;">
													<input type="submit"  name="hkbutton" class="an_input2" value=去付款 "  />
													&nbsp;
