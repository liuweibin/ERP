...
$(document).ready(function() {
	$('#StreamSumbit').click(function() {
		streamquery(0);
									});
});

function streamquery(pageno){
			var url_ = "accountRecord.do?type=accountStreamQuery";
								url_ += bindStreamData();
								url_ += "&pageno=" + pageno;
												...
												var data=page.result;
													...
													for ( var i = 0; i < data.length; i++) {
													    ...
													    $("#rowsPerPage").val(data[i].rowsPerPage);
													     document.getElementById("streamBuy").innerHTML=data[i].sumRevenue;
													     document.getElementById("streamTo").innerHTML=data[i].sumExpenses;
													}
													if (tab != '') {
													...
													$("#dataStreamTable tr").hover(
													 ...
													 //加入分页的绑定
														...
														$("#PaginationStream").pagination(total, {
...
/*
function search_() {
	var url_ = "accountRecord.do?type=accountStreamQuery&conditions=";
	url_ += bindLargeData();
	//url_ += "&fields=" + fields;
	url_ += "&rowDisplayed=" + $("#rowsPerPage").val();
	alert(url_);
	parent.main.location.href = url_;
}*/

function bindStreamData() {
	 var temp=$("#tabCot_product_2").is(":visible");
	 if(temp){
		var conditions = "&tranType=serial";
		conditions += "&tranTypeVal=" + $("#tranTypeVal").val();
		...
		document.getElementById("queryDate").disabled=false;
	}
}
 ...
 function chkclick(chkid) {
    if (chkid == 0) {
        if ($("#show_all").attr("checked") == true) {
            ...
            $("#show_to10").removeAttr("checked");
            $("#show_to11").removeAttr("checked");
        }
    } else {
...
<input type="hidden" id="rowsPerPage" />
					...
					<li>
	         		   ...
	         		   <input id="StreamSumbit" type="button" class="Partorange"  value="查询"/>
					</li>
				</ul>
	...
	<table class="tablelist pusht" style="margin-top: 20px; width: 100%;" >
		<tr style="width: 100%;">
			<th height="23" class="title_0" style="display: none" style="width: 9%;">
							交易流水号
						</th>
						<th style="width: 8%;">
							变更编号
	...
	<div id="pages">
		<div id="PaginationStream" class="pagination"></div>
	</div>
